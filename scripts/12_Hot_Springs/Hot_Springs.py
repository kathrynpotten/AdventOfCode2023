    ...
    for new_index, next_item in enumerate(row[index + 1 :]):
        ...
        if total_springs == grouping:
            if type(row[index_count]) == int:
                ...
                # print(f"the {grouping} fits here")
                row[index] = grouping
                ...
                # print(row, set_indices_dict, index_tally, index)
                break
            ...
            if next_item + total_springs == grouping:
                ...
                # print(row, set_indices_dict, index_tally)
                break
            ...
            if new_index == len(row[index + 1 :]) - 1:
                ...
                if total_springs == grouping:
                    ...
                    # print(row)
                    set_indices_dict[grouping].append(index + index_tally)
                    # print(set_indices_dict)

    return row, order, index_tally, set_indices_dict


def possible_configurations(input_row, order, original_row, final_indices=set()):
    # print("\n")
    # print(input_row)
    possible = False
    ...
    for i in range(len(spring_row)):
        ...
        if i == len(spring_row):
            ...
            if len(order) == 0:
                ...
                grouping = order[0]
                # if grouping > 1:
                # print(f"grouping is {grouping},", item, index, spring_row)
                ...
                if all(spring_row[index + x] == "?" for x in range(grouping)):
                    ...
                    if index + grouping == len(spring_row):
                        ...
                        order = order[1:]
                        set_indices_dict[grouping].append(index + index_tally)
                        # print(set_indices_dict, spring_row)
                        ...
                        index_tally += grouping - 1
                        # print(set_indices_dict, index_tally, index, spring_row)
                    else:
                        spring_row[index] = "."

                else:
                    # print("it might fit but there are things in the way")
                    # print(index_tally, index)
                    total_springs = 1
                    (
                        spring_row,
        ...
        for spring_set in spring_sets:
            ...
            if (
                ...
                (
                    ...
                    new_input_row, original_order, original_row, final_indices
                )
                if possible:
...
def test_arrangement(input_row, order, original_row, final_indices=set()):
    (
        output_arrangement,
        order_residual,
        possible,
        set_indices_dict,
        final_indices,
    ) = possible_configurations(input_row, order, original_row, final_indices)

    # print(input_row, output_arrangement, order_residual)
...
def loop_over_set_indices(
    set_indices, temp_row, original_row, spring_row, order, possibilities, seen
):
...
def sum_of_arrangements(input_strings):
    ...
    for num, springs in enumerate(input_strings):
        ...
        sum += possible
        if springs not in tested and (possible == 1 and num > 463):
            #    # print("\n")
